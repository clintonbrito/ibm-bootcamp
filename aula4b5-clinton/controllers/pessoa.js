...
const buscarTodasPessoas = (requisicao, resposta) => {
    const querySQL = 'SELECT p.idpessoa as id, p.nomepessoa as nome, p.idadepessoa as idade, p.emailpessoa as email, p.genero_idgenero as genero FROM pessoa p';
...
const inserirPessoa = (requisicao, resposta) => {
    ...
    if(!pessoa.id){
        const querySQL = `INSERT INTO pessoa (nomepessoa, idadepessoa, emailpessoa, genero_idgenero) VALUES (?, ?, ?, ?)`;
        const valores = [pessoa.nome, pessoa.idade, pessoa.email, pessoa.genero];
...
const atualizarPessoa = (requisicao, resposta) => {
    ...
    if(pessoa.id){
        ...
        if(pessoa.genero){
            querySQL+= `genero_idgenero = ?,`;
            valores.push(pessoa.genero);
        }
